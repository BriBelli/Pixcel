/* global React, ReactDOM, PG, IDERail, IDEPrimaryNav, IDETopBar, IDECanvas, IDEIcon, IDEModelTrigger, IDEAgentPanel,
   ...
   IDEAssetsPanel, IDEResizeHandle, IDEWorkflowLanding,
   IDEPageHeader,
...
(function () {
  ...
  function App() {
    ...
    var dkState = useState(false); var dockOpen = dkState[0], setDockOpen = dkState[1];
    ...
    /* workflow phase: landing intro → working IDE. enterWorking() runs the
       transition once, on the first card pick or agent message. */
    var phState = useState('landing'); var phase = phState[0], setPhase = phState[1];
    var lvState = useState(false); var leaving = lvState[0], setLeaving = lvState[1];
    var enState = useState(false); var justEntered = enState[0], setJustEntered = enState[1];
    function enterWorking() {
      ...
      setDockOpen(true);
      setJustEntered(true);
      setTimeout(function () { setJustEntered(false); }, 640);
      setTimeout(function () { setLeaving(false); }, 700);
    }
    ...
    var dock = dockOpen
      ? h(Dock, { key: 'dock', tab: tab, setTab: setTab, split: split, collapsed: collapsed, setCollapsed: setCollapsed,
          ...
          side: t.dockSide, entering: justEntered, onCollapse: function () { setDockOpen(false); } })
    ...
    /* tier-2 per-workflow tool sub-rail (the MVP bar) — sits right of the primary section nav */
    var rail = section === 'image' && h(IDERail, { key: 'rail', labeled: t.railLabels });
    var ease = 'cubic-bezier(0.22, 1, 0.36, 1)';
    var fade = t.entry === 'fade';
    var landOut = fade ? 'pxs-screen-out 300ms ' + ease + ' forwards' : 'pxs-land-up-out 440ms ' + ease + ' forwards';
    var workIn = fade ? 'pxs-screen-in 420ms ' + ease + ' 90ms both' : 'pxs-work-up-in 520ms ' + ease + ' 70ms both';
    var showLanding = phase === 'landing' || leaving;
    var workLayer = phase === 'working' && h('div', { key: 'work', style: { position: 'absolute', inset: 0,
      animation: leaving ? workIn : 'none' } },
      canvasMode !== 'focus' && h(IDEFloatingPrompt, { fields: fields, setField: setField, model: model, picker: picker,
        analysis: analysis, assembled: assembled, onGenerate: generate, onJump: jump }));
    var landLayer = showLanding && h('div', { key: 'land', style: { position: 'absolute', inset: 0,
      animation: leaving ? landOut : 'none', pointerEvents: leaving ? 'none' : 'auto' } },
      h(IDEWorkflowLanding, { section: section, onPick: function () { enterWorking(); } }));
    var canvasWrap = h('div', { key: 'cw', style: { flex: 1, minWidth: 0, position: 'relative', display: 'flex', minHeight: 0 } },
      h('div', { style: { flex: 1, minWidth: 0, position: 'relative', overflow: 'hidden', background: 'var(--pxs-bg-canvas)',
    ...
    var agent = h(IDEAgentPanel, { key: 'agent', open: agentOpen, setOpen: setAgentOpen, model: model, width: agentW,
      onSubmit: function () { enterWorking(); } });
    ...
    return h('div', { className: 'pxs-scope', style: { position: 'fixed', inset: 0, display: 'flex', flexDirection: 'row',
      ...
      h(TweaksPanel, null,
        ...
        h(TweakSection, { label: 'Workflow' }),
        h(TweakRadio, { label: 'Enter transition', value: t.entry, options: ['chat', 'fade'],
          onChange: function (v) { setTweak('entry', v); } }),
  ...
  function Dock(props) {
    ...
    var dockEase = 'cubic-bezier(0.22, 1, 0.36, 1)';
    var dockAnim = props.entering ? ((props.side === 'left' ? 'pxs-dock-in-l ' : 'pxs-dock-in-r ') + '460ms ' + dockEase + ' both') : 'none';
    return h('aside', { key: 'dock', style: { width: props.width, flexShrink: 0, height: '100%', background: 'var(--a2ui-bg-app)',
      ...
      display: 'flex', flexDirection: 'column', minHeight: 0, animation: dockAnim } },
