...
// ============================================
// Pixcel (PXC) Component Types
// ============================================

/**
 * The core Pixcel data model (`PXSFrame` / `PXSCell` / `PXSAnimation` and the
 * compressed/on-chain forms) is owned by `@pxs/core` — there is ONE canonical
 * definition. a2ui-core re-exports it so PXC A2UI components share the exact
 * same shapes as the Studio. Do NOT redeclare these here.
 *
 * @see packages/pxs-core/src/types/pxs-types.d.ts
 */
export type {
  PXSCell,
  PXSFrame,
...
/**
 * A frame delta (sparse cell patch). a2ui-core-local: there is no canonical
 * `@pxs/core` equivalent yet — it is built on top of the shared `PXSCell`.
 */
export interface PXSDelta {
  patch: import('@pxs/core/types/pxs-types').PXSCell[];
}

export interface PxcCanvasProps {
  frame: PXSFrame | string;
  interactive?: boolean;
  zoom?: number;
  showGrid?: boolean;
  resolution?: string;
...
export interface PxcGalleryProps {
  frames: PXSFrame[];
  columns?: number;
  selectable?: boolean;
}

export interface PxcFrameDeckProps {
  animation: PXSAnimation;
  autoplay?: boolean;
  showControls?: boolean;
}

export interface PxcExportProps {
  formats: ('png' | 'jpeg' | 'gif' | 'webp' | 'svg' | 'mp4' | 'pxs-json')[];
  frameRef: string;
}

export interface PxcInspectorProps {
  frame: PXSFrame;
...
export interface A2UIBuiltinComponents {
  ...
  // Pixcel (PXC)
  'pxc-canvas': PxcCanvasProps;
  'pxc-gallery': PxcGalleryProps;
  'pxc-frame-deck': PxcFrameDeckProps;
  'pxc-export': PxcExportProps;
  'pxc-inspector': PxcInspectorProps;
}
