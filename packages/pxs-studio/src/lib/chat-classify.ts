...
- "transfer" (LARGE / heavy) — oriented; the job has DEPTH (a project, a film/video scene, iteration, character consistency). Hand off to a specialist: set frame = { goal, subject, medium } ONLY. Do NOT write an image prompt — the specialist owns the prompt and all image specs. In your opener refer to it as the **image agent** / an **image specialist** (it creates the images) — NOT a "motion" or "video" specialist, even for a video scene. Even a video scene starts from a reference image — say why in your opener.
