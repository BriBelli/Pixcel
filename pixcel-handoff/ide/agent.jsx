      ...
      <div style={{ flexShrink: 0, padding: 12, borderTop: '1px solid var(--pxs-border-subtle)', background: 'var(--a2ui-cool-900)' }}>
        <div style={{ background: 'var(--a2ui-bg-input)', border: '1px solid var(--a2ui-border-default)', borderRadius: 12, padding: 9 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginBottom: 8 }}>
            ...
            <ContextChip icon={<IDEIcon name="layers" size={12} />} removable>Jonny 1 · sheet</ContextChip>
            <ContextChip icon={<IDEIcon name="sparkles" size={12} />} removable>Prompt</ContextChip>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 8 }}>
            ...
            <div style={{ flex: 1 }} />
            <button title="Send" disabled={!val.trim()}
      ...
      <aside style={{ width: props.width || 376, flexShrink: 0, height: '100%', background: 'var(--a2ui-bg-app)',
        ...
        <div style={{ flexShrink: 0, borderBottom: '1px solid var(--pxs-border-subtle)', background: 'var(--a2ui-cool-900)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '0 12px 10px' }}>
            ...
            <span style={{ fontSize: 11, color: 'var(--a2ui-text-tertiary)', fontFamily: 'var(--a2ui-font-mono)' }}>Jonny 1 · 8 views</span>
            <div style={{ flex: 1 }} />
        ...
        {/* thread */}
        <div className="ide-scroll" style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '16px 14px', display: 'flex', flexDirection: 'column', gap: 18 }}>

          {/* — Ask exchange (Q&A, read-only) — */}
          <UserMsg context={[
            <ContextChip key="p" icon={<IDEIcon name="sparkles" size={12} />}>Prompt</ContextChip>,
          ]}>Which part of this prompt is weakest for keeping Jonny consistent across all 8 views?</UserMsg>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <AsstHead brain={brain} mode="Ask" time="3:56 PM" />
            <Para>
              Your subject pins two anchors — <b style={{ color: 'var(--a2ui-text-primary)', fontWeight: 600 }}>amber visor glow</b> and <b style={{ color: 'var(--a2ui-text-primary)', fontWeight: 600 }}>etched silver seams</b>. That holds, since {model.name} drops anchors only past ~4 named details.
            </Para>
            <Para>The thinner risk is <b style={{ color: 'var(--a2ui-text-primary)', fontWeight: 600 }}>Style</b>: “muted teal tones” reads differently on the low-angle view. Pin the exact grade and key light in every frame so the sheet stays uniform.</Para>
          </div>

          {/* — Agent exchange (multi-step, tool-using) — */}
          <UserMsg context={[
            <ContextChip key="s" icon={<IDEIcon name="layers" size={12} />}>Jonny 1 · sheet</ContextChip>,
          ]}>Add three dynamic action poses and grade the whole sheet cooler — keep him consistent.</UserMsg>

          <div style={{ display: 'flex', flexDirection: 'column', gap: 11 }}>
            <AsstHead brain={brain} mode="Agent" time="3:59 PM" />
            <Para>On it. I’ll extend the sheet with three action views, hold the anchors, then push a cooler grade across every frame.</Para>

            <Plan done={2} steps={[
              { state: 'done', title: 'Read prompt + reference sheet', sub: '8 views · 2 consistency anchors' },
              { state: 'done', title: 'Lock anchors', sub: 'amber visor glow · etched silver seams' },
              { state: 'run', title: 'Generate 3 action poses', sub: 'on ' + model.name },
              { state: 'pending', title: 'Apply cooler grade to all frames' },
            ]} />

            <ToolRow name="read_sheet" arg="Jonny 1" icon="layers" />

            <ToolRow name="generate_views" icon="aperture" open
              input={'{\n  "model": "' + model.id + '",\n  "poses": ["mid-stride", "combat ready", "leaping"],\n  "anchors": ["amber visor glow", "etched silver seams"],\n  "match": "Jonny 1"\n}'}>
              <ResultGrid tiles={['Mid-stride', 'Combat ready', 'Leaping']} />
            </ToolRow>

            <ToolRow name="apply_grade" arg="cooler · all 11 views" icon="contrast" running />

            <StreamLine>Three poses are in and consistent. Now regrading the full sheet toward cool — I’ll keep the amber visor as the one warm accent so it still reads</StreamLine>
          </div>
        </div>
